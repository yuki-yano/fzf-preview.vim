function! fzf_preview#mr#append(path, cache_path) abort
  let files = s:get_files_with_create_directory(a:cache_path)

  call insert(files, a:path)
  call writefile(fzf_preview#util#uniq(files), a:cache_path)
endfunction

function! fzf_preview#mr#mru_file_path() abort
  return g:fzf_preview_cache_directory . '/mru'
endfunction

function! fzf_preview#mr#mrw_file_path() abort
